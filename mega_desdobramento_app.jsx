...
export default function MegaDesdobramentoApp() {
  ...
  /* -------------------------
     ...
     - tenta o /ultimo para obter número do último concurso
     - depois busca N concursos para trás (até 50)
  ...
  async function fetchDraws() {
    ...
    try {
      ...
      if (resUltimo.ok) {
        ...
        // Guidi retorna { numero: <int>, dezenas: [...] } ou similar
    ...
    for (let n = ultimo; n > ultimo - maxBack; n--) {
      try {
        ...
        // network/CORS or parse error - skip
        continue;
      }
    }
  ...
  function toggleNumber(n) {
    setCopied(false);
    setInput(""); // clear manual input to avoid confusion (we sync below)
    setNumbers((prev) => {
      const exists = prev.includes(n);
      if (exists) return prev.filter((x) => x !== n);
      const next = [...prev, n].sort((a, b) => a - b);
      // limit 20 max selection (same as validation)
      if (next.length > 20) return prev;
      return next;
    });
  }

  // sync numbers -> input (string)
  useEffect(() => {
    if (numbers && numbers.length) {
      setInput(numbers.join(" "));
    } else {
      // if user cleared numbers, don't overwrite a typed input
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <p className="mb-3 text-sm text-gray-700">
        Selecione números no volante (clique) ou digite no campo. Depois clique em <strong>Gerar Desdobramento</strong>.
      </p>
      ...
      {/* amostra de combos */}
      <div className="mb-6">
        <h3 className="font-semibold">Amostra (primeiras 200):</h3>
      ...
      {/* resultados do concurso verificado */}
      ...
      {/* numeros provaveis */}
      {numerosProvaveis && numerosProvaveis.length > 0 && (
        <div className="mb-6">
      ...
      <div className="text-xs text-gray-500 mt-6">
        ...
        pode ser bloqueada por CORS; nesse caso rode um pequeno proxy ou backend que repasse as requisições.
      </div>
    </div>
  );
}
